...
pretrained_dn_model_path = 'pretrainedModel/Dn_model_rgb2rgb.pth'
pretrained_dm_color_model_path = 'pretrainedModel/pretrained_tcpd_color_unet_model.pth'
pretrained_dm_polar_model_path = 'pretrainedModel/pretrained_tcpd_polar_unet_model.pth'

save_dir = 'results'

config_dn = {
    "name": "SubModelDnRgb2Rgb",
    "in_nc": 12,
    "out_nc": 12,
    "nf": 64
}
...
model_dn = model.Model(config_dn)
model_dm = model.Model(config_dm)
...
for input_path, label_path_list in zip(input_list, label_list):
    ...
    model_dn.load_state_dict(dn_checkpoint['model_state_dict'])
    
    # Load DM models (these are direct state dicts)
    model_dm.color_unet.load_state_dict(torch.load(pretrained_dm_color_model_path, map_location=device))
    model_dm.polar_unet.load_state_dict(torch.load(pretrained_dm_polar_model_path, map_location=device))
    ...
    model_dm.eval()
    with torch.no_grad():
        # output_x = model_dn(input_x)

        # output_x = shape_control.channel_separate_to_dense_raw_torch(output_x).unsqueeze(1)
        # output_x, colors = model_dm(output_x)

        input_x = shape_control.channel_separate_to_dense_raw_torch(input_x).unsqueeze(1)

        print(f"input_x shape before DM: {input_x.shape}")  # Expected: [1, 1, 4, 3, H, W]
        output_x, colors = model_dm(input_x)
        print(f"output_x shape after DM: {output_x.shape}")  # Expected: [1, 4, 3, H, W]
        
        # Stack 4 polarization angles along dim=1 to get [1, 12, H, W]
        # Reshape from [1, 4, 3, H, W] to [1, 12, H, W]
        batch_size, num_angles, channels, height, width = output_x.shape
        output_x_stacked = output_x.view(batch_size, num_angles * channels, height, width)
        print(f"output_x_stacked shape for DN: {output_x_stacked.shape}")  # Expected: [1, 12, H, W]
        
        # Apply DN model on stacked data
        output_x_dn = model_dn(output_x_stacked)
        print(f"output_x_dn shape after DN: {output_x_dn.shape}")  # Expected: [1, 12, H, W]
        
        # Split back from [1, 12, H, W] to [1, 4, 3, H, W]
        output_x = output_x_dn.view(batch_size, num_angles, channels, height, width)
        print(f"output_x shape after splitting: {output_x.shape}")  # Expected: [1, 4, 3, H, W]
        print(f"output_x max: {output_x.max()}, output_x min: {output_x.min()}")
