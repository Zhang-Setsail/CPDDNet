...
@register_model('SubModelDn')
class SubModelDn(BaseModel):
    """
    ...
    def __init__(self, config):
        ...
        self.dn_pool4 = nn.MaxPool2d(kernel_size=2)

        self.dn_conv5_1 = nn.Conv2d(dn_nf * 8, dn_nf * 16, kernel_size=3, stride=1, padding=1)
        self.dn_conv5_2 = nn.Conv2d(dn_nf * 16, dn_nf * 16, kernel_size=3, stride=1, padding=1)

        self.dn_upv6 = nn.ConvTranspose2d(dn_nf * 16, dn_nf * 8, 2, stride=2)
        self.dn_conv6_1 = nn.Conv2d(dn_nf * 16, dn_nf * 8, kernel_size=3, stride=1, padding=1)
        self.dn_conv6_2 = nn.Conv2d(dn_nf * 8, dn_nf * 8, kernel_size=3, stride=1, padding=1)

        self.dn_upv7 = nn.ConvTranspose2d(dn_nf * 8, dn_nf * 4, 2, stride=2)
        self.dn_conv7_1 = nn.Conv2d(dn_nf * 8, dn_nf * 4, kernel_size=3, stride=1, padding=1)
        self.dn_conv7_2 = nn.Conv2d(dn_nf * 4, dn_nf * 4, kernel_size=3, stride=1, padding=1)

        self.dn_upv8 = nn.ConvTranspose2d(dn_nf * 4, dn_nf * 2, 2, stride=2)
        self.dn_conv8_1 = nn.Conv2d(dn_nf * 4, dn_nf * 2, kernel_size=3, stride=1, padding=1)
        self.dn_conv8_2 = nn.Conv2d(dn_nf * 2, dn_nf * 2, kernel_size=3, stride=1, padding=1)

        self.dn_upv9 = nn.ConvTranspose2d(dn_nf * 2, dn_nf, 2, stride=2)
        self.dn_conv9_1 = nn.Conv2d(dn_nf * 2, dn_nf, kernel_size=3, stride=1, padding=1)
        self.dn_conv9_2 = nn.Conv2d(dn_nf, dn_nf, kernel_size=3, stride=1, padding=1)

        self.dn_conv10_1 = nn.Conv2d(dn_nf, dn_out_nc, kernel_size=1, stride=1)
    ...
    def forward(self, x):
        ...
        dn_pool4 = self.dn_pool4(dn_conv4)

        # 编码器第5层（底部）
        dn_conv5 = self.relu(self.dn_conv5_1(dn_pool4))
        ...
        dn_up6 = torch.cat([dn_up6, dn_conv4], 1)
        ...
        dn_up7 = torch.cat([dn_up7, dn_conv3], 1)
        ...
        # 解码器第8层
        dn_up8 = self.dn_upv8(dn_conv7)
        dn_up8 = torch.cat([dn_up8, dn_conv2], 1)
        dn_conv8 = self.relu(self.dn_conv8_1(dn_up8))
        # dn_conv8_identity = dn_conv8
        dn_conv8 = self.relu(self.dn_conv8_2(dn_conv8))
        # dn_conv8 = dn_conv8 + dn_conv8_identity  # 残差连接

        # 解码器第9层
        dn_up9 = self.dn_upv9(dn_conv8)
        dn_up9 = torch.cat([dn_up9, dn_conv1], 1)
        dn_conv9 = self.relu(self.dn_conv9_1(dn_up9))
        # dn_conv9_identity = dn_conv9
        dn_conv9 = self.relu(self.dn_conv9_2(dn_conv9))
        # dn_conv9 = dn_conv9 + dn_conv9_identity  # 残差连接

        # 输出层
        out = self.dn_conv10_1(dn_conv9)
